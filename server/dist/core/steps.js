...
export function inferVariableCosts(input) {
    const totalFixed = input.FixedCapitalCosts + input.FixedLaborCosts;
    const variableCosts = Math.max(input.Sales - input.Profit - totalFixed - input.Amortization - input.Interests, 0);
...
export function distributeSurplus(distributableSurplus, weights) {
    return {
        capitalReturn: distributableSurplus * weights.weightCapital,
        laborSurplusPool: distributableSurplus * weights.weightLabor,
    };
}
export function allocateLaborSurplus(laborSurplusPool, employees = []) {
    if (!employees.length)
        return [];
...
export function auditSystem(data) {
    ...
    if (monetaryValues.some((value) => value < 0))
        return "FAIL";
    if (data.input.Sales < totalFixed)
        return "FAIL";
    if (data.variableCosts.VariableCosts < 0)
        return "FAIL";
    const weightSum = data.weights.weightCapital + data.weights.weightLabor;
    if (Math.abs(weightSum - 1) > epsilon)
        return "FAIL";
    if (Math.abs(sumDistrib - data.distributableSurplus) > epsilon)
        return "FAIL";
    if (Math.abs(sumEmployee - data.distribution.laborSurplusPool) > epsilon)
        return "FAIL";
    return "PASS";
}
