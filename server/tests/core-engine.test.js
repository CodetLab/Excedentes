...
describe("EXCEDENTES Core Engine orchestrator", () => {
    ...
    const getAllocationAmount = (entry) => entry.amount ?? entry.allocation ?? entry.value ?? 0;
    const assertAuditAndCertificate = (result, expectedStatus) => {
    ...
    const assertInvariants = (result) => {
        const sumEmployeeAllocations = (result.employeeSurplusLedger || []).reduce((total, entry) => total + getAllocationAmount(entry), 0);
        ...
        expect((result.capitalReturn || 0) + (result.laborSurplusPool || 0)).toBeCloseTo(result.distributableSurplus || 0, 6);
